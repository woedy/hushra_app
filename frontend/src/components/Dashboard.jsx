          ...
          <div className="p-6 flex-1 overflow-y-auto custom-scrollbar">
                ...
                <div className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${
                  ...
                  <div className="flex gap-2">
                    {autoEnabled ? (
                      ...
                      <button onClick={handleToggleAutoRun} disabled={isToggling || !autoStates.length}
                    ...
                    <button onClick={handleSeedNow} disabled={isSeedingNow || !autoStates.length}
            ...
            {activeTab === 'jobs' && (
              <div className="flex flex-col gap-4">
                <p className="text-xs text-gray-500 uppercase tracking-widest font-bold">Active Jobs</p>
                <div className="space-y-3">
                  {jobs.filter(j => j.status === 'RUNNING' || j.status === 'STOPPED').length === 0 && <p className="text-sm text-gray-500 italic">No jobs currently running or stopped.</p>}
                  {jobs.filter(j => j.status === 'RUNNING' || j.status === 'STOPPED').slice(0, 5).map(j => (
