...
export default function AutoRunManager() {
  ...
  const loadSettings = useCallback(async () => {
    ...
    rows.forEach(item => {
      ...
      if (item.key === 'auto_run_states') setAutoStates(item.value ? item.value.split(',').map(s => s.trim()).filter(Boolean) : []);
    ...
    <div className="space-y-6">
      {!credentialsReady && (
        <div className="bg-red-500/10 border border-red-500/40 rounded-xl p-4">
          ...
          <p className="text-xs text-red-200/90 mt-1">You need at least one active UUID credential before Auto Run tasks can start.</p>
        </div>
      )}
            ...
            <div className="grid grid-cols-10 gap-1 max-h-28 overflow-y-auto">
              {ALL_STATES.map(st => (
                <button key={st} onClick={() => setAutoStates(prev => prev.includes(st) ? prev.filter(s => s !== st) : [...prev, st])}
                  className={`text-[10px] py-1 rounded border ${autoStates.includes(st) ? 'bg-blue-500/20 border-blue-500 text-blue-300' : 'bg-gray-900 border-gray-700 text-gray-500'}`}>
                  {st}
                </button>
              ))}
            </div>
