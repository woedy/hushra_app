...
class GlobalSettingViewSet(viewsets.ModelViewSet):
    ...
    def orchestrator_status(self, request):
        ...
        usable_credentials = HushraCredentials.objects.filter(is_active=True, request_count__lt=soft_limit).count()
    ...
    def seed_now(self, request):
        ...
        if not HushraCredentials.has_usable_credentials(soft_limit=soft_limit):
            return Response({'error': 'No usable UUID credentials are available right now. Reset the UUID pool or increase soft limit.'}, status=400)
