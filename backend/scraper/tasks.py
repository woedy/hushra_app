...
def execute_ssn_lookup(self, task_id):
    ...
    if not credential:
        ...
        task.status = 'PENDING'
        task.save(update_fields=['status'])
        raise self.retry(countdown=getattr(settings, "HUSHRA_NO_CREDENTIAL_RETRY_SECONDS", 300))
    ...
    if client is None:
        if login_err == "AUTH_FAILED":
            ...
            raise self.retry(countdown=getattr(settings, "HUSHRA_RATE_LIMIT_RETRY_SECONDS", 300))
        else:
            ...
            raise self.retry(countdown=min(backoff_cap, (2 ** self.request.retries) * 30))
